...
public class DeferredCompositor implements Cleanupable {
	...
	protected void renderOutlines(CacheManager cache, Vector2i resolution, boolean needRegen) {
	...
	protected void blitToScreen(CacheManager cache, Vector2i resolution, boolean needRegen) {
		blitShader.bind();
		if (needRegen) {
	...
	protected void renderWorldScene(CacheManager cache, WorldLevelScene worldScene, Vector2i resolution, boolean needRegen) {
	...
	protected void renderScene(Scene scene, Map<Class<? extends Renderable>, RenderShader> shaders) {
	...
	private void renderMesh(Mesh mesh, Component component, Entity entity, Matrix4f transformationMatrix, RenderShader shader) {
		...
		if (shader.createUniform(DirectShader.TINT)) {
			if (entity instanceof TintOwner tintEntity) {
				tintOwner = tintEntity;
			} else if (mesh instanceof TintOwner tintMesh) {
				tintOwner = tintMesh;
			} else {
				tintOwner = null;
			}
		...
		if (shader.createUniform(GradientShader.GRADIENT_DIRECTION)) {
			if (entity instanceof GradientOwner gradientEntity) {
				gradientOwner = gradientEntity;
			} else if (mesh instanceof GradientOwner gradientMesh) {
	...
	protected void resizeFramebuffer(Framebuffer fb, Vector2i resolution) {
		fb.bind();
