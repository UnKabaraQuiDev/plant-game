...
public class DeferredIconRenderer implements Cleanupable {
	...
	private static final String ICON_FRAMEBUFFER_NAME = "_ICON_FRAMEBUFFER";
	private static final int ICON_FRAMEBUFFER_POS_IDX = 0;
	private static final int ICON_FRAMEBUFFER_NORMAL_IDX = 1;
	private static final int ICON_FRAMEBUFFER_UV_IDX = 2;
	private static final int ICON_FRAMEBUFFER_IDS_IDX = 3;
	...
	protected Framebuffer iconFramebuffer;
	...
	protected Scene3D fakeScene = new Scene3D("fakeScene");
	...
	public DeferredIconRenderer(GameEngine engine, Thread ownerThread) {
		...
		transferShader = cache.getAbstractShader(TransferShader.class);
		instanceTransferShader = cache.getAbstractShader(InstanceTransferShader.class);

		materialComputeShader = cache.getAbstractShader(MaterialComputeShader.class);
		textureMaterialComputeShader = cache.getAbstractShader(TextureMaterialComputeShader.class);

		outlineShader = cache.getAbstractShader(OutlineShader.class);
	...
	public SingleTexture render(GameEngine engine, GameObject obj, int outputSize, Vector4f lightColor,
		...
		texture.setDataType(outputTxt.getDataType());
		texture.setInternalFormat(outputTxt.getInternalFormat());
		texture.setFormat(outputTxt.getFormat());
		texture.setFilters(TextureFilter.NEAREST);
		texture.setup();

		resizeFramebuffer(iconFramebuffer, renderResolution);
		outputTxt.setSize(renderResolution);
		outputTxt.resize();

		synchronized (fakeScene.getEntitiesLock()) {
			fakeScene.addEntity(obj);

			renderObject(worldSceneShaders, obj, renderResolution);
	...
	private void renderOutlines(CacheManager cache, Vector2i resolution) {
		GL_W.glViewport(0, 0, resolution.x, resolution.y);
		assert GL_W.checkError("Viewport(" + resolution + ")");
		...
		setupOutlineInputs(outlineShader, resolution);
	...
	private void setupOutlineInputs(OutlineShader computeShader, Vector2i resolution) {
		posTexture.bind(0);
	...
	private void renderMaterials(CacheManager cache, Scene3D scene, Vector2i resolution, Vector4f lightColor,
		...
		setupMaterialInputs(materialComputeShader, scene, resolution, lightColor, lightDirection, ambientLight);
		...
		synchronized (scene.getEntitiesLock()) {
	...
	private void setupMaterialInputs(ComputeShader computeShader, Scene3D scene, Vector2i resolution,
		...
		computeShader.setUniform(MaterialComputeShader.LIGHT_COLOR, lightColor);
	...
	private Framebuffer genFramebuffer(final CacheManager cache, Vector2i resolution) {
		final Framebuffer framebuffer = new Framebuffer(ICON_FRAMEBUFFER_NAME);
		framebuffer.gen();
		framebuffer.bind();

		final int width = resolution.x, height = resolution.y;

		depthTexture = new SingleTexture(ICON_FRAMEBUFFER_NAME + ".depth", width, height);
		...
		posTexture = new SingleTexture(ICON_FRAMEBUFFER_NAME + ".pos", width, height);
		...
		framebuffer.attachTexture(FrameBufferAttachment.COLOR_FIRST, ICON_FRAMEBUFFER_POS_IDX, posTexture);

		normalTexture = new SingleTexture(ICON_FRAMEBUFFER_NAME + ".normal", width, height);
		...
		framebuffer.attachTexture(FrameBufferAttachment.COLOR_FIRST, ICON_FRAMEBUFFER_NORMAL_IDX, normalTexture);

		uvTexture = new SingleTexture(ICON_FRAMEBUFFER_NAME + ".uv", width, height);
		...
		framebuffer.attachTexture(FrameBufferAttachment.COLOR_FIRST, ICON_FRAMEBUFFER_UV_IDX, uvTexture);

		idsTexture = new SingleTexture(ICON_FRAMEBUFFER_NAME + ".ids", width, height);
		...
		framebuffer.attachTexture(FrameBufferAttachment.COLOR_FIRST, ICON_FRAMEBUFFER_IDS_IDX, idsTexture);
