	...
	public static TaskFuture<?, InstanceEmitter> getFuture(
		...
		return new TaskFuture<>(loader, (ThrowingSupplier<GenericMeshData, Throwable>) () -> {
			...
			return getStaticMeshData(path);
				...
				.then(render,
						(ThrowingFunction<GenericMeshData, Mesh, Throwable>) obj -> StaticMeshLoader.createStatic(cache, meshName, obj))
				.then(render,
						(ThrowingFunction<Mesh, InstanceEmitter, Throwable>) (
								final Mesh mesh) -> create(cache, name, mesh, bufferSize, transforms));

	}

	static InstanceEmitter create(
			final CacheManager cache,
			final String name,
			final Mesh mesh,
			final int bufferSize,
			final Function<Integer, Transform> transform) {
		final InstanceEmitter te = new InstanceEmitter(
				name,
				mesh,
